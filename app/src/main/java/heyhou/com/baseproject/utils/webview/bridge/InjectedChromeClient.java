...
public class InjectedChromeClient extends WebChromeClient {
    ...
    public void onProgressChanged(WebView view, int newProgress) {
        ...
        Log.d(TAG, "progress----->>>>>>> " + newProgress + " ,isInJect:" + mIsInjectedJS + "  ,Ready:" + mReady);
        if (newProgress < 25) {
            mIsInjectedJS = false;
            mReady = false;
        } else if (!mIsInjectedJS && newProgress == 100) {
            view.loadUrl(mJsCallJava.getPreloadInterfaceJS());
            mIsInjectedJS = true;
//            if (!mReady && newProgress >= 90) {
//                mReady = true;
//                view.loadUrl("javascript:hh.ready()");
//            }
